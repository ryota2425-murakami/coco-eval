...
class COCOeval:
    ...
    def _prepare(self):
        ...
        self.evalImgs = defaultdict(list)   # per-image per-category evaluation recalls
        self.eval     = {}                  # accumulated evaluation results
        ...
        for k, k0 in enumerate(k_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    #print("dtm", dtm.shape,dtm)
                    #(10, 921) [[78. 47. 90. ...  0.  0.  0.]

                    #print("tps",tps.shape,tps)
                    #10 921[[ True  True  True ... False False False]

                    #print("fps",fps.shape,fps)
                    #(10, 921) [[False False False ... False False  True]
                    ...
                    #print( "fps_sum" , fp_sum.shape )
                    # 10 921 [  1.   2.   3. ... 443. 443. 443.]

                
                    #print("tp_sum",tp_sum.shape,tp_sum)
                    #(10, 921) [[  1.   2.   3. ... 443. 443. 443.]
                    ...
                    for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                        ...
                        if m ==2 and a==0 and k ==0:
                            print("=================")
                            print("閾値:",0.5+0.05*int(t))
                            #print(inds)
#                            print("gtIg(TP+FP)",gtIg)
#                            print("npig(TP + FN)",npig)
                            print("総ラベル数")
                            print("FN:", int(npig)- np.amax(tp))
                            print("TP",np.amax(tp))
                            print("FP",np.amax(fp))
                            print("適合率",(np.amax(tp))/(np.amax(tp)+np.amax(fp)))
                            print("再現率",(np.amax(tp))/(np.amax(tp)+(int(npig)- np.amax(tp))))
                            print("正解率",(np.amax(tp))/(np.amax(tp)+np.amax(fp)+(int(npig)- np.amax(tp))))
                            print("F値",(np.amax(tp)*2)/(2*np.amax(tp)+np.amax(fp)+(int(npig)- np.amax(tp))))
                            #配列:検出精度
                            #print("precision",np.array(q))
#                            print("score",np.array(ss))
                            #閾値ごとのtpの配列
                            #print("tp_sum",tp_sum)
...
#                            print("fp_sum",fp_sum)
    ...
    def summarize(self):
        ...
        def _summarizeKps():
            ...
            stats[3] = _summarize(1, maxDets=20, iouThr=.80)
            stats[4] = _summarize(1, maxDets=20, iouThr=.85)
    ...
    def makeplot(self, recThrs, precisions, name, save_dir="./RESULT"): 
        print('save dir create')
        save_dir="./RESULT"
        ...
        for aidx in range(precisions.shape[2]):
            ...
            #print('save figure to !!!!!rr%s,%s'% (save_dir,plotname))
